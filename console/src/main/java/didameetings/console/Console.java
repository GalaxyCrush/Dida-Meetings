...
public class Console {
    private int ballot_completed = 0;
                    ...
                    if ((parameter1 != null) && (parameter2 != null)) {
                        ...
                        if (ballot_number < this.getLastBallot())
                            ...
                            try {
                                ...
                                final int targetReplica = replica;
                                final int finalSequence_number = sequence_number;
                                ...
                                new Thread(new Runnable() {
                                    public void run() {
                                        ...
                                        if (newballot_responses.size() >= 1) {
                                            ...
                                            System.out.println("Sending ACTIVATE to replica " + targetReplica);


                                            // Build the ActivateRequest
                                            DidaMeetingsMaster.ActivateRequest activate = DidaMeetingsMaster.ActivateRequest.newBuilder()
                                                    .setReqid(finalSequence_number * 100 + client_id)
                                                    .setBallot(ballot_number)
                                                    .build();

                                            // Send activate asynchronously without waiting
                                            console_async_stubs[targetReplica].activate(activate, new CollectorStreamObserver<>(new GenericResponseCollector<>(new ArrayList<>(), 1)));
